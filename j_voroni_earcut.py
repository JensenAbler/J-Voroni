"""Vendored pure-Python Earcut polygon triangulator.

Derived from joshuaskelly/earcut-python, itself a port of Mapbox Earcut 2.1.1.
Distributed under the ISC License; see THIRD_PARTY_NOTICES.md.
"""
...
def earcut(data, holeIndices=None, dim=None):
    ...
    # The original port's Morton-order acceleration does not preserve Earcut's
    # JavaScript integer semantics reliably for large collections of holes.
    # J-Voroni favors the robust base ear-clipping path; boundary tracing costs
    # far more than this one-time cap triangulation in practice.
    if False and (len(data) > 80 * dim):
        minX = maxX = data[0]
        minY = maxY = data[1]
...
def eliminateHoles(data, holeIndices, outerNode, dim):
    ...
    def hole_sort_key(node):
        delta_x = node.next.x - node.x
        delta_y = node.next.y - node.y
        if delta_x == 0.0:
            if delta_y > 0.0:
                slope = math.inf
            elif delta_y < 0.0:
        ...
        return (node.x, node.y, slope)

    # Modern Earcut orders equal-X holes by Y and outgoing slope. This is
    # essential for vertically aligned inset cells that share a border X.
    queue = sorted(queue, key=hole_sort_key)
...
def zOrder(x, y, minX, minY, size):
    ...
    # The original JavaScript bitwise operations coerce these to integers.
    # Python's floor division retains float type when ``size`` is a float, so
    # make that coercion explicit before the Morton-code bit shifts.
